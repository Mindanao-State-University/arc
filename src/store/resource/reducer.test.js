...
describe('RESOURCE_LIST_READ_SUCCESS', () => {
  it('sets list in the initial state', () => {
    expect(reducer(
      initialState,
      action(actions.RESOURCE_LIST_READ_SUCCESS, [1, 2, 3], { count: 0 }),
    )).toEqual(state({ list: [1, 2, 3] }))
  })

  it('overrides list in an existing state', () => {
    expect(reducer(
      state({ list: [1, 2, 3], count: 0 }),
      action(actions.RESOURCE_LIST_READ_SUCCESS, [3, 2, 1], { count: 0 }),
    )).toEqual(state({ list: [3, 2, 1], count: 0 }))
  })
})
